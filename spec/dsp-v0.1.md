# Document Sovereignty Protocol — Specification v0.1-draft
...
The Document Sovereignty Protocol (DSP) is an open standard that defines how sensitive
...
DSP does NOT mandate any storage technology. It defines an abstract interface with
...
to the DSP Gateway for dashboard visibility, but replicated events MUST NOT contain
PII.
...
```
            ...
            ┌────────────┼───────────────┐
            ▼            ▼               ▼
      DSP Gateway   Processing Agent   Consumer App
            │            │               │
...
The DSP Gateway orchestrates the protocol but SHOULD NOT accumulate enough data
...
A DSP processing session begins when the gateway creates a session context
...
| Level | Requirements |
|-------|-------------|
| **DSP Core** | Layers 1-4 schemas. Manifests, contracts, results, audit events. |
| **DSP Attested** | DSP Core + hardware attestation (Layer 2 attestation_requirements enforced) |
| **DSP Sovereign** | DSP Attested + customer-managed encryption keys + data residency enforcement |
| **DSP AI-Safe** | DSP Attested + mandatory result scanning + NER + privacy budget + document sanitization |

### 6.2 Progressive Adoption Path

DSP is designed to be adopted incrementally. Each conformance level builds on
the previous one, and implementations can start simple and add capabilities
over time.

#### Getting Started (30 minutes)

1. Install the `dsp-gateway` reference implementation.
...
#### DSP Core (1 day integration)

Requirements:
- Implement or integrate `dsp-gateway` with your storage backend.
...
#### DSP Attested (1 week integration)

Requirements (in addition to DSP Core):
...
#### DSP AI-Safe (2 week integration)

Requirements (in addition to DSP Attested):
...
#### DSP Sovereign (ongoing)

Requirements (in addition to DSP AI-Safe):
...
All messages in a DSP session MUST use the same major version. Implementations
...
| 0.1.3 | Added wire protocol specification (OpenAPI 3.1) — `spec/dsp-api-v0.1.yaml` |
...
The DSP wire protocol is defined in `spec/dsp-api-v0.1.yaml` (OpenAPI 3.1). It
...
- `GET /v0.1/.well-known/dsp-configuration` — Discovery and version negotiation
