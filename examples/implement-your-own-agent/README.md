# Build a DSSP-Compliant Agent from Scratch

A step-by-step tutorial that takes you from zero to a working DSSP agent.
...
```json
{
  ...
  "token": {
    ...
    "scope": {
      "document_ids": ["doc-f47ac10b58cc4372", "doc-a58bd21c69dd5483"],
...
def download_documents(manifest: dict) -> list[tuple[dict, bytes]]:
    ...
    client = Minio(
        ...
        access_key="dssp-owner",
        secret_key="dssp-owner-secret-key",
        secure=False,
    )
...
def redact(value: str, method: str) -> str:
    """Apply a DSSP redaction method to a PII value.

    DSSP defines these methods (see spec §3.4):
      mask_last_4   →  ****4372
...
def build_result_envelope(
    ...
    """Build a schema-compliant DSSP result envelope."""
...
```bash
cd dssp-protocol/reference/validator
...
- **Spec**: `spec/dssp-v0.1.md` — full protocol specification
- **API**: `spec/dssp-api-v0.1.yaml` — OpenAPI 3.1 definition (load in Swagger UI)
- **Schemas**: `schemas/*.schema.json` — JSON Schema for every DSSP artifact
