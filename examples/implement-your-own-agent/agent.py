"""DSP Tutorial Agent — Minimal Compliant Implementation
...
def download_documents(manifest: dict) -> list[tuple[dict, bytes]]:
    ...
    client = Minio(
        MINIO_ENDPOINT,
        access_key=os.environ.get("MINIO_ACCESS_KEY", "dsp-owner"),
        secret_key=os.environ.get("MINIO_SECRET_KEY", "dsp-owner-secret-key"),
        secure=False,
    )
...
def redact(value: str, method: str) -> str:
    """Apply a DSP-spec redaction method."""
    if method == "allow":
        return value
...
def build_envelope(
    ...
    """Assemble a schema-compliant DSP result envelope."""
