"""DSSP AI-Safe — AI-specific safety tests."""
...
class TestAgentTypeScanning:
    def test_llm_freeform_requires_ner(self, sample_contract):
        ...
        assert "ner" in scanners, "NER scanner MUST be required for llm_freeform"
        ...
        for extraction in sample_result.get("extractions", []):
            for key, value in extraction.get("fields", {}).items():
                if isinstance(value, float):
                    # Use str() for faithful representation — avoids
                    # IEEE 754 artifacts from format specifiers like .20g
                    s = str(value)
                    if "." in s:
