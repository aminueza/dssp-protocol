"""DSP Core — Audit chain integrity tests."""
...
class TestAuditChainIntegrity:
    def test_sequential_sequence_numbers(self, sample_audit_trail):
        ...
        events = sample_audit_trail if isinstance(sample_audit_trail, list) else sample_audit_trail.get("events", [sample_audit_trail])
        if len(events) < 2:
    ...
    def test_genesis_event_has_no_previous(self, sample_audit_trail):
        ...
        if events:
            ...
            assert prev is None or prev == "", \
                f"Genesis event should not have previous_event_hash"
...
class TestAuditEventTypes:
    def test_known_event_types(self, sample_audit_trail):
        ...
        known_types = {
            "manifest.created", "manifest.updated", "manifest.expired",
            "contract.created", "contract.updated", "contract.suspended",
            "contract.revoked", "contract.expired",
            "session.started", "session.completed", "session.failed",
            "session.terminated", "session.timeout",
            "document.accessed", "document.processed",
            "result.produced", "result.delivered", "result.rejected",
            "result.quarantined", "result.scan_passed", "result.scan_failed",
            "attestation.verified", "attestation.failed",
            "attestation.end_of_session", "attestation.heartbeat",
            "sanitization.applied", "sanitization.injection_detected",
            "sidecar.network_mismatch", "sidecar.anomaly_detected",
            "privacy_budget.consumed", "privacy_budget.exceeded",
            "violation.detected", "violation.escalated",
            "access.granted", "access.denied", "access.revoked",
            "ledger.genesis", "ledger.checkpoint",
        }
        events = sample_audit_trail if isinstance(sample_audit_trail, list) else sample_audit_trail.get("events", [sample_audit_trail])
        for event in events:
