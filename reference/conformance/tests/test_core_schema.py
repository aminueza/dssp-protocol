"""DSP Core — Schema conformance tests."""
...
class TestManifestSchema:
    ...
    def test_manifest_requires_dsp_version(self, load_schema, sample_manifest, schema_registry):
        schema = load_schema("manifest.schema.json")
        invalid = {k: v for k, v in sample_manifest.items() if k != "dsp_version"}
        with pytest.raises((ValidationError, Exception)):
            _validate(invalid, schema, schema_registry)

    def test_manifest_requires_documents(self, load_schema, sample_manifest, schema_registry):
        schema = load_schema("manifest.schema.json")
...
class TestContractSchema:
    ...
    def test_contract_requires_consumer(self, load_schema, sample_contract, schema_registry):
        schema = load_schema("contract.schema.json")
