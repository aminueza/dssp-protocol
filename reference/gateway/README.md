# DSSP Gateway -- Reference Implementation (Go)

A runnable Go server implementing the Document Sovereignty Protocol & Privacy (DSSP) v0.1 wire protocol.
...
# Or build and run
go build -o dssp-gateway .
./dssp-gateway

# With Docker
docker build -t dssp-gateway .
docker run -p 8080:8080 dssp-gateway
```
...
|----------------------|----------|--------------------------------|
| `DSSP_PORT`           | `8080`   | HTTP listen port               |
| `DSSP_STORAGE_ADAPTER`| `memory` | Storage backend (memory only)  |
| `DSSP_LOG_LEVEL`      | `info`   | Log level (debug/info/warn/error) |
...
| GET    | `/v0.1/.well-known/dssp-configuration`    | DSP configuration        |
...
# 7. Check DSSP configuration
curl http://localhost:8080/v0.1/.well-known/dssp-configuration
```
...
internal/
  types/types.go                 Core DSSP types matching JSON schemas
