...
A minimal DSSP Gateway that demonstrates the full protocol flow:
...
app = FastAPI(
    title="DSSP Gateway",
    description="Document Sovereignty Protocol & Privacy — Reference Gateway",
    version="0.1.0",
)
...
@app.get("/health")
async def health():
    return {"status": "ok", "dssp_version": "0.1", "gateway": "reference-impl"}
...
async def start_session(request: Request):
    ...
    token = {
        "token": f"dssp-tok-{uuid.uuid4().hex}",
        "expires_at": (
    ...
    token["storage_credentials"] = {
        "access_key": os.environ.get("MINIO_AGENT_ACCESS_KEY", "dssp-owner"),
        "secret_key": os.environ.get("MINIO_AGENT_SECRET_KEY", "dssp-owner-secret-key"),
    }
...
# Genesis event
add_audit_event("ledger.genesis", reason="DSSP Gateway reference implementation started")
add_flow_step("🚀", "Gateway started", "DSSP v0.1 reference implementation ready")
...
<title>DSSP Gateway — Reference Dashboard</title>
...
<div class="max-w-7xl mx-auto px-4 py-8">
  ...
  <div class="flex items-center justify-between mb-8">
    <div>
      ...
      <p class="text-slate-400 mt-1">Document Sovereignty Protocol & Privacy — Reference Implementation v0.1</p>
    </div>
  ...
  <div class="text-center text-slate-600 text-sm mt-8">
    DSSP v0.1 · Apache 2.0 · Reference Implementation — Not for production
  </div>
</div>
