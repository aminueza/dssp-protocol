...
// MinIOAdapter implements Adapter using MinIO/S3-compatible storage.
...
// bucket name and object key so callers never see the raw S3 path.
...
func (m *MinIOAdapter) GeneratePresignedURL(ctx context.Context, documentID string, ttlSeconds int) (string, error) {
	...
	reqParams.Set("X-Dssp-Document-Id", documentID)
...
// VerifyIntegrity compares the stored hash against the expected SHA-256 hex value.
// Prefers the Dssp-Sha256 user metadata header, falling back to ETag.
...
// resolveDocumentID walks the bucket/prefix to find the object whose
// deterministic document ID matches the requested ID. This linear scan
// is acceptable for the reference implementation but not for production.
...
func (m *MinIOAdapter) objectInfoToDocumentInfo(info minio.ObjectInfo) DocumentInfo {
	...
	classification := meta["Dssp-Classification"]
	if classification == "" {
		classification = "unclassified"
	}

	sensitivity := meta["Dssp-Sensitivity"]
	if sensitivity == "" {
		sensitivity = "none"
	}
...
func (m *MinIOAdapter) extractHash(info minio.ObjectInfo) string {
	if stored := info.UserMetadata["Dssp-Sha256"]; stored != "" {
		return stored
	}
