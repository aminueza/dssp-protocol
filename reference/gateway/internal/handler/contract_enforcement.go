...
//
// These functions enforce the rules defined in a DSP processing
// contract. They are called by the HTTP handlers before state
// transitions.
// ──────────────────────────────────────────────────────────────
...
func validateSessionRequest(
	...
	// Verify enclave type if attestation is provided.
	...
	// Verify agent hash if contract requires it.
	...
	// Verify agent ID if contract specifies one.
...
func validateHeartbeat(
	...
	if contract.AttestationRequirements.RuntimeVerification != nil {
		...
		if req.Attestation != nil && req.Attestation.Timestamp != "" {
			...
			// Heartbeat required but no attestation provided.
			violations = append(violations, ContractViolation{
...
func enforceResultPolicy(contract *types.Contract, result *types.Result) *ResultValidation {
	...
	// 9. Max result size (if enforceable from the envelope).
...
func checkSubAgentPolicy(rv *ResultValidation, contract *types.Contract, claims types.AttestationClaims) {
	...
	// Check if sub-agents are allowed at all.
