// Package types defines the core DSP protocol types matching the JSON schemas.
...
// Schema URIs for each resource type.
const (
	SchemaManifest   = "https://dsp.dev/schema/manifest/v0.1"
	SchemaContract   = "https://dsp.dev/schema/contract/v0.1"
	SchemaResult     = "https://dsp.dev/schema/result/v0.1"
	SchemaAuditEvent = "https://dsp.dev/schema/audit/v0.1"
)

// ID prefixes used when generating identifiers.
const (
	PrefixManifest = "mf"
	PrefixDocument = "doc"
	PrefixContract = "ct"
...
// HashDigest represents a cryptographic hash value.
...
// Timestamp is an alias for time handling. We serialize to RFC 3339 with Z suffix.
type Timestamp = string

// OwnerInfo identifies the document owner.
...
// Manifest describes documents available for processing.
...
// ManifestScope describes what a manifest covers.
...
// ScopePeriod is a date range.
type ScopePeriod struct {
	From string `json:"from,omitempty"`
	To   string `json:"to,omitempty"`
}

// StorageRef describes where a document is physically stored.
...
// DocumentEntry describes a single document in a manifest.
...
// ManifestSummary provides aggregated statistics.
...
// Contract defines what a processor is allowed to do with documents.
...
// ContractConsumer identifies the processing organization and agent.
...
// SubAgentPolicy controls multi-model composition.
...
// ContractPermissions defines what the consumer can do.
...
// DocumentFilter limits which documents the consumer can access.
...
// ContractRestrictions defines processing constraints.
...
// NetworkPolicy controls network egress.
...
// AllowedDestination is a permitted network endpoint.
...
// ResultPolicy controls what can appear in results.
...
// CustomRedactionPattern is a regex-based redaction rule.
...
// NumericPrecisionPolicy controls numeric output precision.
...
// ResultScanning controls result inspection.
...
// NERModelRequirements specifies NER scanner requirements.
...
// DocumentSanitization controls pre-processing sanitization.
...
// PrivacyBudget controls differential privacy and re-identification risk.
...
// GatewayVisibility controls what the gateway can see.
...
// AttestationRequirements defines enclave verification rules.
...
// RuntimeVerification controls in-session verification.
...
// SidecarVerifier is an independent co-processor.
...
// ScopedToken grants time-limited access to documents.
...
// TokenScope defines what a token permits.
...
// Result is the structured output from a processing agent.
type Result struct {
	Schema                   string                 `json:"$schema"`
	DSPVersion               string                 `json:"dsp_version"`
...
// ResultAttestation is the cryptographic proof of enclave execution.
...
// AttestationClaims holds the runtime claims.
...
// SubAgentAttestation records a sub-agent in the processing pipeline.
...
// DocumentExtraction holds structured data from one document.
// Uses json.RawMessage for maximum flexibility — agents may use varied schemas.
type DocumentExtraction = json.RawMessage

// ExtractedTable holds tabular data (used when strongly-typed access is needed).
...
// TableColumn describes a column in an extracted table.
...
// ClassificationResult holds a document classification.
type ClassificationResult struct {
	Label      string  `json:"label"`
	Confidence float64 `json:"confidence"`
}

// ExtractionValidation holds self-validation results.
type ExtractionValidation struct {
	Passed bool              `json:"passed"`
	Checks []ValidationCheck `json:"checks,omitempty"`
}

// ValidationCheck is a single validation rule result.
...
// PIIHandlingReport declares how PII was handled.
...
// PIIViolation details a compliance violation.
...
// ResultScanReport holds the result scanning output.
...
// ResultScanVerdict is a verdict from one scanner.
...
// ScanFinding is an individual PII detection.
...
// ScanStatistics holds aggregate scan numbers.
...
// EndOfSessionAttestation proves enclave integrity at session end.
...
// ProcessingError is a non-fatal error during processing.
...
// AuditEvent records an operation in the DSP lifecycle.
...
// AuditSubject identifies what the action was performed on.
...
// AuditOutcome describes the result of an action.
...
// ViolationDetails provides context for violation events.
...
// DSPError is the standard error response.
...
// SessionAttestation is the attestation proof submitted with a session request.
...
// CreateSessionResponse is returned from POST /sessions.
...
// HeartbeatRequest is the body for POST /sessions/{id}/heartbeat.
...
// CompleteSessionRequest is the body for POST /sessions/{id}/complete.
...
// GrantAccessRequest is the body for POST /storage/grant-access.
...
// GrantAccessResponse is returned from grant-access.
type GrantAccessResponse struct {
	Token         *ScopedToken      `json:"token"`
	PresignedURLs map[string]string `json:"presigned_urls,omitempty"`
}

// ContractUpdateRequest is the body for PATCH /contracts/{id}.
...
// DSPConfiguration is the well-known configuration document.
...
// Endpoints lists the base paths for each resource.
...
// NowUTC returns the current time formatted as a DSP Timestamp.
func NowUTC() Timestamp {
	return time.Now().UTC().Format(time.RFC3339)
}

// FutureUTC returns a future timestamp offset by the given duration.
