"""DSSP Sidecar Verifier — main entry point.

Runs as a separate process alongside the agent. In production,
this would run in a separate enclave.

Usage:
    dssp-sidecar --session-id ps-abc123 --agent-pid 12345 --gateway http://localhost:8080
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--gateway", default="http://localhost:8080", help="Gateway URL"
    )
    parser.add_argument(
        "--poll-interval",
        type=float,
        default=1.0,
        help="Monitoring poll interval (seconds)",
    )
    parser.add_argument(
        "--output", help="Output file for verification result (default: stdout)"
    )
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if status in ("completed", "failed", "terminated"):
                        print(
                            f"[sidecar] Session {status}. Stopping monitor.", flush=True
                        )
                        break
...
def _verify_and_report(evidence, args) -> None:
    ...
    for mismatch in verification.mismatches:
        try:
            audit_event = {
                "event_type": mismatch.get(
                    "audit_event_type", "sidecar.anomaly_detected"
                ),
                "actor": {"type": "system", "agent_id": "dssp-sidecar-verifier"},
    ...
    print(
        f"[sidecar] Verification {status}: {len(verification.mismatches)} mismatch(es)",
        flush=True,
    )
