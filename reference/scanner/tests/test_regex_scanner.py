...
import pytest
from dssp_scanner.regex_scanner import RegexScanner
...
def _make_result(fields: dict) -> dict:
    """Create a minimal DSSP result envelope with the given fields."""
    return {
        "extractions": [
            {
...
class TestRegexScanner:
    ...
    def test_allowed_field_not_flagged(self, scanner):
        ...
        contract = {
            "restrictions": {
                "result_policy": {"pii_redaction_rules": {"IBAN": "allow"}}
            }
        }
