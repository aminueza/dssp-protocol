...
def main() -> None:
    parser = argparse.ArgumentParser(description="DSP Reference Result Scanner")
    parser.add_argument("result", type=Path, help="Path to DSP result envelope JSON")
    parser.add_argument("--contract", type=Path, help="Path to DSP contract JSON (optional)")
    parser.add_argument(
        "--scanners",
        nargs="+",
    ...
    parser.add_argument("--output", type=Path, help="Output file for scan verdicts (default: stdout)")
    parser.add_argument("--quiet", action="store_true", help="Only output JSON, no status messages")
