...
class ScanFinding:
    ...
    field_path: str          # JSONPath to the field (e.g., "extractions[0].fields.account_holder")
...
class ResultScanner(ABC):
    """Base class for DSP result scanners."""
    ...
    def scan(self, result: dict, contract: dict | None = None) -> ScanVerdict:
        """
        Scan a DSP result envelope for PII leakage.

        Args:
            result: The DSP result envelope (parsed JSON).
        ...
        for i, extraction in enumerate(result.get("extractions", [])):
                ...
                for k, row in enumerate(table.get("rows", [])):
                    for col_idx, cell in enumerate(row):
                        if isinstance(cell, str):
                            col_name = table.get("columns", [{}])[col_idx].get("name", f"col{col_idx}") if col_idx < len(table.get("columns", [])) else f"col{col_idx}"
                            values.append((
            ...
            for j, cls in enumerate(extraction.get("classifications", [])):
                ...
                if isinstance(label, str):
                    values.append((f"extractions[{i}].classifications[{j}].label", label))
    ...
    def _extract_numeric_values(self, result: dict) -> list[tuple[str, float]]:
        ...
        for key in ["processing_duration_ms", "memory_peak_bytes", "network_egress_bytes"]:
        ...
        for i, ext in enumerate(result.get("extractions", [])):
            if "confidence" in ext:
                values.append((f"extractions[{i}].confidence", float(ext["confidence"])))
