// Package minio implements a DSSP storage adapter backed by MinIO S3-compatible storage.
//
...
// each one to a DSSP DocumentEntry.  An optional filter narrows results.
...
// objectInfoToEntry converts a MinIO ObjectInfo into a DSSP DocumentEntry.
func (a *Adapter) objectInfoToEntry(info miniogo.ObjectInfo) DocumentEntry {
	...
	if stored := meta["Dssp-Sha256"]; stored != "" {
		hash.Value = stored
	} else {
...
func (a *Adapter) GrantAccess(
	...
	for _, docID := range documentIDs {
		...
		reqParams.Set("X-Dssp-Agent", agentID)
