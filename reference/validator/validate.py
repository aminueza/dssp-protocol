...
SCHEMA_MAP = {
    "https://dsp.dev/schema/manifest/v0.1": "manifest.schema.json",
    "https://dsp.dev/schema/contract/v0.1": "contract.schema.json",
    "https://dsp.dev/schema/result/v0.1": "result.schema.json",
    "https://dsp.dev/schema/audit/v0.1": "audit-event.schema.json",
    "https://dsp.dev/schema/storage-binding/v0.1": "storage-binding.schema.json",
    "https://dsp.dev/schema/common/v0.1": "common.schema.json",
}
...
def build_resolver() -> RefResolver:
    """Build a JSON Schema resolver that can resolve $ref across DSP schemas."""
    store = {}
    for schema_id, filename in SCHEMA_MAP.items():
        path = SCHEMA_DIR / filename
        if path.exists():
            with open(path) as f:
                store[schema_id] = json.load(f)
        # Also store with filename for relative refs
        store[filename] = store.get(schema_id, {})

    # Use common schema as the base
...
def validate_result_pii(data: dict, contract: dict | None, result: ValidationResult) -> None:
    ...
    if pii_report.get("raw_content_included", False):
        if contract:
            allowed = contract.get("restrictions", {}).get("result_policy", {}).get("raw_content_allowed", False)
            if not allowed:
                result.error("Result contains raw content but contract does not allow it.")
        else:
    ...
    for extraction in data.get("extractions", []):
        ...
        scan_for_pii(fields, f"extractions[{extraction.get('source_document_id', '?')}].fields", result)
...
def validate_numeric_precision(data: dict, contract: dict, result: ValidationResult) -> None:
        ...
        for table in extraction.get("tables", []):
            for ri, row in enumerate(table.get("rows", [])):
                for ci, cell in enumerate(row):
                    check_value(cell, f"tables[{table.get('name', '?')}].rows[{ri}][{ci}]")


def validate_sub_agent_chain(data: dict, contract: dict, result: ValidationResult) -> None:
    ...
    if not policy.get("allowed", True):
        if chain:
            result.error("Sub-agents are not allowed by contract but sub_agent_chain is present.")
            return
...
def validate_result_contract_compliance(data: dict, contract: dict, result: ValidationResult) -> None:
    ...
    # Check operations are within contract permissions
    allowed_ops = set(contract.get("permissions", {}).get("operations", []))
    # (operations aren't directly in the result, but we check attestation claims)
...
def validate_file(filepath: Path, contract_path: Path | None = None, check_chain: bool = False) -> ValidationResult:
    ...
    if schema_id == "https://dsp.dev/schema/manifest/v0.1":
        validate_manifest_pii(data, result)

    elif schema_id == "https://dsp.dev/schema/result/v0.1":
        contract = None
        if contract_path:
            try:
...
def validate_directory(dirpath: Path, check_chain: bool = True) -> list[ValidationResult]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="DSP Reference Validator — validates messages against DSP schemas"
    )
    parser.add_argument("path", help="JSON file or directory to validate")
    parser.add_argument("--contract", help="Contract file for result compliance checking")
    parser.add_argument("--check-chain", action="store_true", help="Verify audit chain integrity")
    ...
    if args.all or path.is_dir():
        ...
        results = [validate_file(path, contract_path=contract_path, check_chain=args.check_chain)]
