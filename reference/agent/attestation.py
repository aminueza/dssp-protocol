"""DSP Attestation Abstraction Layer
...
class Attestor(ABC):
    ...
    def get_enclave_type(self) -> str:
        """Return the enclave type string for DSP schemas."""
...
class GramineAttestor(Attestor):
    ...
    def _compute_measurement(self) -> str:
        ...
        if self._is_real_sgx:
            try:
                # Real SGX: read MRENCLAVE from the attestation report
    ...
    def _build_sgx_dcap_quote(self, user_data: bytes = b"") -> bytes:
        ...
        header += bytes.fromhex("939a7233f79c4ca9940a0db3957f0607")  # qe_vendor_id (Intel)
        ...
        report_body += sha256_hex(b"mrsigner-simulated").encode()[:32].ljust(32, b"\x00")  # mrsigner
        ...
        report_body += hashlib.sha256(user_data).digest()      # report_data (first 32 bytes)
        report_body += sha256_hex(
            json.dumps({"agent": self.agent_id, "ts": now_utc()}).encode()
        ).encode()[:32].ljust(32, b"\x00")                     # report_data (next 32 bytes)

        report_body = report_body[:self.SGX_REPORT_BODY_SIZE].ljust(
            self.SGX_REPORT_BODY_SIZE, b"\x00"
        )

        # Simulated signature (ECDSA placeholder)
    ...
    def _generate_simulated_sgx_report(self, user_data: bytes) -> AttestationReport:
        ...
        sig = base64.b64encode(
            hashlib.sha256(quote[:self.SGX_QUOTE_HEADER_SIZE + self.SGX_REPORT_BODY_SIZE]).digest()
        ).decode()
        ...
        return AttestationReport(
            ...
            platform_certificate_chain=[
                ...
                base64.b64encode(b"SIMULATED Intel PCK Certificate - NOT REAL").decode(),
            ],
            raw_quote=raw_quote_b64,
...
class NitroAttestor(Attestor):
    ...
    def _compute_measurement(self) -> str:
        if self._is_real_nitro:
            ...
            # For now, compute from agent files
            pass
        agent_path = Path("/app/main.py")
        if agent_path.exists():
