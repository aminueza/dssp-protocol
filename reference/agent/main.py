...
Document processing agent with pluggable enclave attestation:
  ...
  7. Produces a DSSP result envelope with full attestation
...
def parse_bank_statement(text: str) -> dict:
    ...
    m = re.search(
        r"^([\w\s.]+)\s*-\s*(?:ACCOUNT STATEMENT|REKENINGOVERZICHT)", text, re.MULTILINE
    )
    if m:
        fields["bank_name"] = m.group(1).strip()

    # Transactions are pipe-delimited: Date|Description|Amount|Balance
...
def process_document(
    ...
    """Process a single document: parse, extract, redact, and return extraction."""
    ...
    for tx in parsed["transactions"]:
        ...
        name_match = re.search(
            r"(?:from|to)\s+([A-Z][a-z]+(?: (?:van |de |den |M\. )?[A-Z][a-z.]+)+)",
            desc,
        )
        if name_match:
            ...
            desc = (
                desc[: name_match.start(1)] + redacted_name + desc[name_match.end(1) :]
            )
...
def main() -> None:
    print("=" * 60)
    print("DSSP Reference — Enclave Agent")
    print("=" * 60)
    ...
    # Include raw hardware quote and certificate chain for gateway verification.
    if attestation.get("raw_quote"):
        session_att["raw_quote"] = attestation["raw_quote"]
    if attestation.get("platform_certificate_chain"):
        session_att["platform_certificate_chain"] = attestation[
    ...
    minio_access = os.environ.get("MINIO_ACCESS_KEY", "dssp-owner")
    minio_secret = os.environ.get("MINIO_SECRET_KEY", "dssp-owner-secret-key")
    minio_client = Minio(
        MINIO_ENDPOINT,
    ...
    for v in scan_report["verdicts"]:
        ...
        print(
            f"  {status} {v['scanner_type']}: {v['verdict']} ({v['findings_count']} findings)"
        )
    ...
    sub_agent_chain = [
        {
            ...
            "agent_id": "ag-ae50a72e10000001",
            "agent_hash": {
                "algorithm": "sha-256",
                "value": sha256_bytes(b"text-parser-1.0"),
            },
            "agent_version": "1.0.0",
            "purpose": "ocr",
        ...
        },
        {
            ...
            "agent_id": "ag-1a700dde2ec10002",
            "agent_hash": {
                "algorithm": "sha-256",
                "value": sha256_bytes(b"layout-detector-2.1"),
            },
            "agent_version": "2.1.0",
        ...
        },
        {
            "step_index": 2,
            "agent_type": "ml_structured",
            "agent_id": "ag-7ab1ee503ac10003",
            "agent_hash": {
                "algorithm": "sha-256",
                "value": sha256_bytes(b"table-extractor-3.0"),
            },
            "agent_version": "3.0.2",
    ...
    # Proves enclave wasn't tampered with during the session
