"""Demo DSP agent — demonstrates the full processing lifecycle."""
...
def main() -> None:
    log("Starting DSP demo agent")
    log(f"Gateway: {GATEWAY_URL}")

    # Wait for gateway to be ready
    for i in range(30):
        try:
            resp = requests.get(f"{API_BASE}/.well-known/dsp-configuration", timeout=2)
            if resp.status_code == 200:
                config = resp.json()
                log(f"Gateway ready — DSP version {config.get('dsp_version', 'unknown')}")
                break
    ...
    log("\n=== Demo complete ===")
    log("The DSP lifecycle has been demonstrated:")
